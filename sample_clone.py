...
class HVSsample:
    ...
    #@propagate
    def propagate(self, potential, dt=0.01*u.Myr, threshold=None, plot = False):
        '''
        ...
        for i in range(self.size):
            ...
            if plot == True:
                ...
                namelist = ['t', 'x', 'y', 'z', 'v_x', 'v_y', 'v_z']
                data_table = Table(data=datalist, names=namelist)
                data_table.write('flight'+str(i)+'_Cart.fits', overwrite=True)
        if plot == True: 
            plt.plot(R, zpos)
        ...
        #data_table = Table(data=datalist, names=namelist)
        #data_table.write('coord.fits', overwrite=True)
  ...
  #          datalist=[ts,x,y,z,r,vx,vy,vz,v,L,[self.lnlike[i]]*len(y)]
    ...
    def _load(self, path):
        ...
        namelist = ['r0', 'phi0', 'theta0', 'v0', 'phiv0', 'thetav0', 'm', 'tage', 'tflight', 'ra', 'dec', 'pmra',
                    'pmdec', 'dist', 'vlos', 'GRVS', 'V', 'G', 'par', 'e_par', 'e_pmra', 'e_pmdec', 'GCdist', 'GCv', 'theta', 'phi']

   
        default_units = {'r0': u.pc, 'phi0': u.rad, 'theta0':u.rad, 'v0':u.km/u.s, 'phiv0':u.rad, 'thetav0':u.rad, 'm':u.solMass, 'tage':u.Myr,'tflight':u.Myr,'ra':u.deg,'dec':u.deg,'pmra':u.mas/u.yr, 'pmdec':u.mas/u.yr,'dist':u.kpc,'vlos':u.km/u.s,'GRVS':None,'V':None,'G':None,'par':1e-6*u.arcsec,'e_par':1e-6*u.arcsec,'e_pmra':u.mas/u.yr,'e_pmdec':u.mas/u.yr,'GCdist':u.kpc,'GCv':u.km/u.s, 'theta':u.rad,'phi':u.rad}
